...
function StatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    "LIVE": "bg-[#0E7C86]/20 text-[#0E7C86]",
    "UAT": "bg-[#D96C3F]/20 text-[#D96C3F]",
    "PILOT": "bg-[#16324A]/20 text-[#16324A]",
...
export default async function ProjectPage({ params }: { params: PageParams }) {
  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="container-wrapper">
          ...
          <div className="max-w-3xl">
            <div className="flex flex-wrap items-center gap-3 mb-4">
              <span className="text-[#0E7C86] text-sm">{project.industry}</span>
            ...
            {project.previewUrl && (
              <a
                ...
                className="inline-flex items-center gap-2 text-[#0E7C86] mt-6 hover:underline"
              >
      ...
      {/* Quick facts */}
      <section className="py-12 bg-[#F1F5F7] border-b border-[#1D2730]/10">
        <div className="container-wrapper">
          <div className="grid sm:grid-cols-3 gap-6 max-w-4xl">
            <div>
              <span className="text-[#1D2730]/50 text-sm">Problem</span>
              <p className="text-[#1D2730] mt-1">{project.problem}</p>
            </div>
            <div>
              <span className="text-[#1D2730]/50 text-sm">System built</span>
              <p className="text-[#1D2730] mt-1">{project.systemBuilt}</p>
            </div>
            <div>
              <span className="text-[#1D2730]/50 text-sm">Key workflow</span>
              <p className="text-[#1D2730]/60 text-sm mt-1">{project.workflow}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="container-wrapper">
          <div className="max-w-3xl mx-auto">
            <div className="prose prose-lg">
              <h2 className="text-2xl font-bold text-[#1D2730] mb-4">Overview</h2>
              <p className="text-[#1D2730]/60 leading-relaxed mb-8">
                {project.content.overview}
              </p>

              <h2 className="text-2xl font-bold text-[#1D2730] mb-4">The Challenge</h2>
              <p className="text-[#1D2730]/60 leading-relaxed mb-8">
                {project.content.challenge}
              </p>

              <h2 className="text-2xl font-bold text-[#1D2730] mb-4">The Solution</h2>
              <p className="text-[#1D2730]/60 leading-relaxed mb-8">
                {project.content.solution}
              </p>

              <h2 className="text-2xl font-bold text-[#1D2730] mb-4">Outcomes</h2>
              <ul className="space-y-3">
                {project.content.outcomes.map((outcome, index) => (
                  <li key={index} className="flex items-start gap-3">
                    <CheckCircle className="w-5 h-5 text-[#0E7C86] shrink-0 mt-0.5" />
                    <span className="text-[#1D2730]/60">{outcome}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      {/* Metric disclaimer */}
      <section className="py-8 bg-[#F1F5F7]">
        <div className="container-wrapper">
