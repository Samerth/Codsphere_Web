...
export function BlogSidebar() {
  ...
  return (
    <div className="w-full bg-black rounded-[13px] p-4 md:p-5 lg:p-[20px] relative">
      ...
      <div className="relative z-10">
        ...
        {formStatus.type !== 'idle' && (
          <div className={`mb-4 p-3 rounded-lg text-sm font-sequel ${
            formStatus.type === 'success' ? 'bg-green-900/50 text-green-200 border border-green-700' :
            formStatus.type === 'error' ? 'bg-red-900/50 text-red-200 border border-red-700' :
            formStatus.type === 'loading' ? 'bg-blue-900/50 text-blue-200 border border-blue-700' : ''
          }`}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 md:space-y-5 lg:space-y-[24px]">
          ...
          {selectedFile ? (
            ...
            >
              <svg className="w-5 h-5 md:w-[22px] md:h-[22px] lg:w-6 lg:h-6" viewBox="0 0 24 24" fill="none">
                <path 
          ...
          <label className="flex items-start gap-2 cursor-pointer">
            <input
              ...
              className="w-4 h-4 mt-0.5 rounded border-gray-500 bg-white/10"
              disabled={formStatus.type === 'loading'}
            />
            <span className="text-gray-300 font-sequel text-[14px] md:text-[16px] lg:text-[20px] leading-[18px] md:leading-[20px] lg:leading-[24px] font-normal">
          ...
          >
            <div className="absolute left-3 md:left-4 lg:left-[16px] w-[38px] h-[38px] md:w-[46px] md:h-[46px] lg:w-[51px] lg:h-[51px] bg-black rounded-full flex items-center justify-center">
              {formStatus.type === 'loading' ? (
                <svg className="animate-spin h-5 w-5 md:h-6 md:w-6 lg:h-7 lg:w-7 text-blue-400" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                </svg>
              ) : (
                <svg className="w-[20px] h-[19px] md:w-[24px] md:h-[23px] lg:w-[29px] lg:h-[28px]" viewBox="0 0 29 28" fill="none">
